...
class WeChat{
	...
	**/
	...
	/**
		...
		*@scene：场景编号
	**/
	public function _getQRCode($expires_secords = 604800, $type = "temp", $scene = 1){
    ...
    public function responseMsg()
    {
		...
		// $postStr = $GLOBALS["HTTP_RAW_POST_DATA"];//获得用户发送信息php7无法使用
	...
	private function _doEvent($postObj){ //事件处理
		switch($postObj->Event){
			...
			case 'unsubscribe': //取消订阅
				...
				break;
			case 'CLICK':
        $this->_dosomeorder($postObj);
        break;
			default:;
		}
	}
  private function _dosomeorder($postObj){
		if ($postObj->EventKey == 'chat') {
		$textTpl = "<xml>
					...
					$MsgType = "text";
					$cont='终于成功啦';
					$str = sprintf($textTpl,$postObj->FromUserName,$postObj->ToUserName,time(),$MsgType,$cont);
			 		//还可以保存用户的信息到数据库
			 		echo $str;
    } else if ($postObj->EventKey == 'ticket') {
		$textTpl =	"<xml>
						<ToUserName><![CDATA[%s]]></ToUserName>
						<FromUserName><![CDATA[%s]]></FromUserName>
						<CreateTime>%s</CreateTime>
						...
						$media_id='6fT5GuiKWMfK5Hh9lDZh20Sb1N2c8MB1R2Q5EL035_HYI9NBwb1FaVEpfIC9kknn';
						$str = sprintf($textTpl,$postObj->FromUserName,$postObj->ToUserName,time(),$media_id);
            echo $str;
		}
    ...
    $cont = '欢迎您关注！你可以发送‘笑话’为你讲笑话，还可以陪聊哦';
		$str = sprintf($tpltext,$postObj->FromUserName,$postObj->ToUserName,time(),$cont);
		//还可以保存用户的信息到数据库
		echo $str;
